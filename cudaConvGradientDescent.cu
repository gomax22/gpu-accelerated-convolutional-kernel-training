...
__global__ void cudaConvolve_(double *input, int inputWidth, 
                                double *kernel, int kernelHeight, int kernelWidth,
                                double *output, int outputHeight, int outputWidth, 
                                double bias) {
    // pay attention that large kernels could not be fit into shared memory (and consequently, large fraction of input)
    int index = blockDim.x * blockIdx.x + threadIdx.x;
    if (index < outputHeight * outputWidth) {
        // perform dot product
        double value = 0.f;
        for(int i = 0; i < kernelHeight; i++) { 
            for(int j = 0; j < kernelWidth; j++) { 
                value += (input[(blockIdx.x + i) * inputWidth + (threadIdx.x + j)] * kernel[i * kernelWidth + j]);
            }
        }
        output[index] = value + bias;
    }
}
void cudaConvolve(double *input, int inputHeight, int inputWidth, 
                    double *kernel, int kernelHeight, int kernelWidth,
                    double *output,  int outputHeight, int outputWidth, 
                    double bias) {
    dim3 nBlocks(outputHeight), nThreadsPerBlock(outputWidth);
    double *deviceInput, *deviceKernel, *deviceOutput;
    
    // device vars allocation
    gpuErrchk(cudaMalloc((void **) &deviceInput, inputHeight * inputWidth * sizeof(double)));
    gpuErrchk(cudaMalloc((void **) &deviceKernel, kernelHeight * kernelWidth * sizeof(double)));
    gpuErrchk(cudaMalloc((void **) &deviceOutput, outputHeight * outputWidth * sizeof(double)));

    // data transfer from ram (host) to global memory (device) 
    gpuErrchk(cudaMemcpy(deviceInput, input, inputHeight * inputWidth * sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(deviceKernel, kernel, kernelHeight * kernelWidth * sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemset(deviceOutput, 0, outputHeight * outputWidth * sizeof(double)));

    cudaConvolve_ <<< nBlocks, nThreadsPerBlock >>> (deviceInput, inputWidth, deviceKernel, kernelHeight, kernelWidth, deviceOutput, outputHeight, outputWidth, bias);
    gpuErrchk(cudaGetLastError());
    gpuErrchk(cudaDeviceSynchronize());

    // data transfer from global memory (device) to ram (host)
    gpuErrchk(cudaMemcpy(output, deviceOutput, outputHeight * outputWidth * sizeof(double), cudaMemcpyDeviceToHost));
...
void training(struct Dataset *trainingSet, double *kernel, int kernelHeight, int kernelWidth, int epochs, double lr, int epochStep) {
    ...
    struct timeval cudaConvolveStart, cudaConvolveEnd;
    ...
    struct timeval cudaGradientStart, cudaGradientEnd;
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (int idx = 0; idx < trainingSet->nImages; idx++) {
            ...
            double *cudaOutput = (double *) malloc(height * width * sizeof(double)); memCheck(cudaOutput);
            ...
            gettimeofday(&cudaConvolveStart, NULL);
            cudaConvolve(paddedData, paddedHeight, paddedWidth,
                        kernel, kernelHeight, kernelWidth,
                        cudaOutput, height, width, 0);
            gettimeofday(&cudaConvolveEnd, NULL);
            ...
            */
            ...
            applyActivationFunction(cudaOutput, height, width, sigmoid);
            ...
            double *errors = computeErrors(cudaOutput, label, height, width); memCheck(errors);
            ...
            // compute gradient
            double *cudaGradient = (double *) malloc(kernelHeight * kernelWidth * sizeof(double)); memCheck(cudaGradient);
            
            
            gettimeofday(&cudaGradientStart, NULL);
            cudaConvolve(paddedData, paddedHeight, paddedWidth, 
                        errors, height, width,
                        cudaGradient, kernelHeight, kernelWidth, 0);
            gettimeofday(&cudaGradientEnd, NULL);
            /*printf("CONVOLVE (BP): ");
            ...
            */
            ...
            update(kernel, cudaGradient, kernelHeight, kernelWidth, lr);
        ...
        if (epoch == 0 || (epoch+1) % epochStep == 0) {
            ...
            printf("(cudaConv: %.2es, activ: %.2es, errors: %.2es, loss: %.2es, cudaGrad: %.2es, update: %.2es)\n", 
                (double) ((cudaConvolveEnd.tv_sec - cudaConvolveStart.tv_sec) + (cudaConvolveEnd.tv_usec - cudaConvolveStart.tv_usec) / 1000000.0f),
                ...
                (double) ((cudaGradientEnd.tv_sec - cudaGradientStart.tv_sec) + (cudaGradientEnd.tv_usec - cudaGradientStart.tv_usec) / 1000000.0f),
...
void predict(struct Dataset *testSet, double *kernel, int kernelHeight, int kernelWidth, int interactive) {
    ...
    for (int idx = 0; idx < testSet->nImages; idx++) {
        ...
        // allocate feature maps (output of the conv layer)
        double *cudaPreds = (double *) malloc(height * width * sizeof(double)); memCheck(cudaPreds);
        ...
        cudaConvolve(paddedData, paddedHeight, paddedWidth, 
                    kernel, kernelHeight, kernelWidth,
                    cudaPreds, height, width, 0);
        gettimeofday(&cudaConvolveEnd, NULL);
        
        // activations
        gettimeofday(&applyActivationFunctionStart, NULL);
        applyActivationFunction(cudaPreds, height, width, stepFunction);
        gettimeofday(&applyActivationFunctionEnd, NULL);

        if (interactive) { printf("\nPREDS: \n"); printArray(cudaPreds, height, width); }
        
        // get accuracy
        gettimeofday(&getAccuracyStart, NULL); 
        getAccuracy(cudaPreds, label, height, width, &accuracy, &nCorrect);
        ...
        printf("(cudaConv: %.2es, activ: %.2es, accuracy: %.2es)\n", 
            (double) ((cudaConvolveEnd.tv_sec - cudaConvolveStart.tv_sec) + (cudaConvolveEnd.tv_usec - cudaConvolveStart.tv_usec) / 1000000.0f),
            (double) ((applyActivationFunctionEnd.tv_sec - applyActivationFunctionStart.tv_sec) + (applyActivationFunctionEnd.tv_usec - applyActivationFunctionStart.tv_usec) / 1000000.0f),
